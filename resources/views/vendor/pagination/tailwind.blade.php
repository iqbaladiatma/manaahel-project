    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="flex gap-2 items-center justify-between sm:hidden">
            ...
            @if ($paginator->onFirstPage())
                <span class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 dark:text-gray-500 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border cursor-not-allowed leading-5 rounded-xl">
                    {!! __('pagination.previous') !!}
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border leading-5 rounded-xl hover:bg-gray-50 dark:hover:bg-dark-border hover:border-blue-primary dark:hover:border-gold focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-gold/20 transition-all duration-150">
                    {!! __('pagination.previous') !!}
                </a>
            @endif

            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border leading-5 rounded-xl hover:bg-gray-50 dark:hover:bg-dark-border hover:border-blue-primary dark:hover:border-gold focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-gold/20 transition-all duration-150">
                    {!! __('pagination.next') !!}
                </a>
            @else
                <span class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 dark:text-gray-500 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border cursor-not-allowed leading-5 rounded-xl">
