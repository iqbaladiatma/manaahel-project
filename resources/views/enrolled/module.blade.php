<x-app-layout>
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-50 dark:bg-dark-bg mt-16">
        <!-- Sidebar - Module List -->
        <div class="w-full lg:w-96 bg-white dark:bg-dark-card border-r border-gray-200 dark:border-dark-border flex-shrink-0 lg:h-[calc(100vh-4rem)] lg:sticky lg:top-16 overflow-y-auto shadow-xl dark:shadow-dark-border">
            <div class="p-6 border-b border-gray-200 dark:border-dark-border bg-gradient-to-br from-blue-50 via-white to-amber-50">
                <a href="{{ route('enrolled.show', $program->slug) }}" class="group inline-flex items-center text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-gold transition-colors mb-4 font-semibold">
                    <svg class="w-5 h-5 mr-2 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                    {{ __('Back to Dashboard') }}
                </a>
                <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">{{ $course->title }}</h2>
                
                <!-- Progress Bar -->
                <div class="mt-4">
                    <div class="flex justify-between text-sm text-gray-600 dark:text-gray-400 mb-2 font-medium">
                    ...
                    <a href="{{ route('enrolled.module.show', [$program->slug, $course->slug, $mod->id]) }}" 
                       class="group/item flex items-start p-5 hover:bg-gradient-to-r hover:from-blue-50 hover:to-amber-50 transition-all duration-300 {{ $isActive ? 'bg-gradient-to-r from-blue-50 to-amber-50 border-l-4 border-blue-600' : 'border-l-4 border-transparent' }}">
                        <div class="flex-shrink-0 mr-4 mt-0.5">
                            ...
                            @else
                                <div class="w-8 h-8 border-2 border-gray-300 dark:border-dark-border rounded-lg flex items-center justify-center text-sm text-gray-500 dark:text-gray-500 font-semibold group-hover/item:border-blue-400 group-hover/item:text-blue-600 transition-colors">
                                    {{ $index + 1 }}
                                </div>
                            @endif
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-base font-semibold {{ $isActive ? 'text-blue-700' : 'text-gray-700 dark:text-gray-300' }} line-clamp-2 group-hover/item:text-blue-600 transition-colors">
                                {{ $mod->title }}
                            </p>
                            <p class="text-sm text-gray-500 dark:text-gray-500 mt-1 flex items-center">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            <!-- Content Section -->
            <div class="flex-1 p-8 overflow-y-auto bg-gradient-to-br from-gray-50 to-white dark:from-dark-card dark:to-dark-bg">
                <div class="max-w-6xl mx-auto">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                        <div class="flex-1">
                            ...
                            @if($module->duration_minutes)
                                <div class="flex items-center text-gray-600 dark:text-gray-400 text-base">
                    ...
                    @if($module->video_url)
                        <div class="mb-10">
                            <div class="bg-white dark:bg-dark-card rounded-2xl shadow-xl dark:shadow-dark-border border border-gray-200 dark:border-dark-border overflow-hidden">
                                <div class="bg-gradient-to-r from-blue-50 to-amber-50 px-6 py-4 border-b border-gray-200 dark:border-dark-border">
                                    ...
                                    @else
                                        <div class="flex items-center justify-center py-16 text-white">
                                            <div class="text-center">
                                                <svg class="w-16 h-16 mx-auto mb-4 text-gray-600 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    @if($module->description)
                        <div class="bg-white dark:bg-dark-card rounded-2xl shadow-lg dark:shadow-dark-border border border-gray-200 dark:border-dark-border p-8 mb-8">
                            <div class="flex items-start">
                                ...
                                <div class="flex-1">
                                    <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-3">{{ __('About This Lesson') }}</h3>
                                    <p class="text-gray-700 dark:text-gray-300 text-lg leading-relaxed">{{ $module->description }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($module->content)
                        <div class="bg-white dark:bg-dark-card rounded-2xl shadow-lg dark:shadow-dark-border border border-gray-200 dark:border-dark-border p-8 mb-8">
                            <div class="flex items-center mb-6">
                                ...
                                <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">{{ __('Lesson Content') }}</h3>
                            </div>
                    ...
                    @if(!$module->video_url && !$module->description && !$module->content)
                        <div class="bg-white dark:bg-dark-card rounded-2xl shadow-lg dark:shadow-dark-border border border-gray-200 dark:border-dark-border p-16 text-center">
                            ...
                            <p class="text-gray-600 dark:text-gray-400 text-lg max-w-md mx-auto leading-relaxed">{{ __('The lesson materials for this module are being prepared. Please check back later.') }}</p>
                        </div>
                    @endif

                    <!-- Navigation Buttons -->
                    <div class="bg-white dark:bg-dark-card rounded-2xl shadow-lg dark:shadow-dark-border border border-gray-200 dark:border-dark-border p-6 mt-8">
                        @php
                        ...
                        <div class="flex items-center justify-between gap-4">
                            ...
                            @if($nextModule)
                                <a href="{{ route('enrolled.module.show', [$program->slug, $course->slug, $nextModule->id]) }}" 
                                   class="group flex items-center px-8 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-300 shadow-xl dark:shadow-dark-border hover:shadow-2xl dark:hover:shadow-gold/20 transform hover:-translate-y-0.5">
                            ...
                            @else
                                <a href="{{ route('enrolled.show', $program->slug) }}" 
                                   class="group flex items-center px-8 py-4 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-bold rounded-xl hover:from-green-600 hover:to-emerald-600 transition-all duration-300 shadow-xl dark:shadow-dark-border hover:shadow-2xl dark:hover:shadow-gold/20 transform hover:-translate-y-0.5">
                                    <svg class="w-6 h-6 mr-2" fill="currentColor" viewBox="0 0 20 20">
